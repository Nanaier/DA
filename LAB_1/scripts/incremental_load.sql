...
WHERE NOT EXISTS(
        ...
        WHERE dt.team_code = t.team_id and dt.team_name = t.team_name and dt.former_team_names = t.former_team_names
          and dt.current = t.current and dt.former = t.former and dt.defunct = t.defunct and dt.first_appearance = t.first_appearance );
...
WHERE NOT EXISTS(
        SELECT winner_id, season_code, season_year_id, winner_name, count_teams
        ...
        WHERE ds.winner_id = (select team_id from football_wr.DimTeams dt where dt.team_name = fss.winner)
          and ds.season_code = fss.season_id and
              ds.season_year_id = (select dy.year_id from football_wr.DimYear dy where dy.year = fss.season)
          and ds.winner_name = fss.winner and ds.count_teams = fss.count_teams );
